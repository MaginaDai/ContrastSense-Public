...
parser.add_argument('-penalty', default=False, type=float, help='Use penalty or not')
parser.add_argument('-penalty_lambda', default=10, type=float, help='penalty para')
...
def main():
    ...
    model = ContrastSense_model(transfer=True, out_dim=args.out_dim, classes=5, dims=args.d, 
    ...
    with torch.cuda.device(args.gpu_index):
        ...
        if args.evaluate:
            contrast.test_performance_cross_dataset(args.model_dir, test_loader_for_all_datasets, datasets)
            return
        if args.penalty:
            contrast.transfer_train_penalty(tune_loader, val_loader, fisher)
        else:
            contrast.transfer_train(tune_loader, val_loader)
        best_model_dir = os.path.join(contrast.writer.log_dir, 'model_best.pth.tar')
        contrast.test_performance_cross_dataset(best_model_dir, test_loader_for_all_datasets, datasets)
