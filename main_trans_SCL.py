...
parser.add_argument('--pretrained', default='Origin_w_HASC', type=str,
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=int(args.bcl), shuffle=True,
    ...
    model = MoCo_model(transfer=True, out_dim=args.out_dim, classes=ClassesNum[args.name], dims=args.d, 
                       classifier_dim=args.classifer_dims, final_dim=args.final_dim, momentum=args.mo, drop=args.drop, DAL=args.DAL, 
                       users_class=user_num, SCL=args.SCL)
    ...
    with torch.cuda.device(args.gpu_index):
        ...
        if args.evaluate:
            ...
            return
        moco.transfer_train_SCL(tune_loader, val_loader, train_loader)
