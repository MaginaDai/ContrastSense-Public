...
def obtain_held_out_based_on_ContrastSense_setting(version, dataset="Shoaib"):
    domain = np.load(f"../dataset/{dataset}_{version}.npz")
    domain_id = np.concatenate([domain['val_domain'], domain['test_domain']])
    all_domain = []
    for i in domain_id:
        domain_name = f"Participant_{i}"
...
def train(dataset_full, args):
    ...
    held_out_users = obtain_held_out_based_on_ContrastSense_setting(args.version)
    # pdb.set_trace()
    ...
    working_directory = os.path.join("./runs", args.store, "Shoaib")
    if not os.path.exists(working_directory):
...
## Training hyperparameters
    ...
    # %%
        if args.training_mode == 'multi':
            ...
            if not os.path.exists(f"{working_directory}/model_best.hdf5"):
                trained_model_save_path = f"{working_directory}/model_best.hdf5"
                trained_model_low_loss_save_path = f"{working_directory}/model_best_lowest.hdf5"
...
def obtain_fine_tuning_data_based_on_ContrastSense_setting(version, shot=10):
    data_loc = f"../../../datasets/Shoaib/"
    fine_tune_file = f"../../../datasets/Shoaib_{version}/tune_set_{shot}.npz"
    data = np.load(fine_tune_file)
    fine_tune_data = data['tune_set']
    ...
    for i in fine_tune_data:
        data_sample_dir = data_loc + f"{i}"
        sample = np.load(data_sample_dir, allow_pickle=True)
        acc, gyro, label = sample['acc'], sample['gyro'], sample['add_infor'][0]
        sensor = np.expand_dims(np.concatenate((acc, gyro), axis=1), axis=0)
        x.append(sensor)
        y.append(label)
    # pdb.set_trace()
    ...
    val_file = f"../../../datasets/Shoaib_{version}/val_set.npz"
...
def fine_tune_evaluate(dataset_full, trained_model_save_path, args):
    ...
    working_directory = os.path.join("./runs", args.store, f"Shoaib_ft_shot_{args.shot}")
    ...
    if args.eval_mode != 'none':
        ...
        test_x = []
        test_y = []
        for device in dataset_full.device_user_ds.keys():
            for u in sorted(dataset_full.device_user_ds[device].keys()):
                if u in held_out_users:
                    test_x.append(dataset_full.device_user_ds[device][u][0])
                    test_y.append(dataset_full.device_user_ds[device][u][1])
        np_test_x = np.concatenate(test_x, axis=0)
        np_test_y = np.concatenate(test_y, axis=0)
        # pdb.set_trace()
        ...
        if args.eval_mode == 'base_model':
            ...
            ## obtain all the labeled data and labels 

            training_set_device_X, training_set_device_y, val_set_x, val_set_y = obtain_fine_tuning_data_based_on_ContrastSense_setting(args.ft_version, args.shot)
            ...
            training_set_device_y = tf.keras.utils.to_categorical(training_set_device_y, num_classes=output_shape)
            val_set_y = tf.keras.utils.to_categorical(val_set_y, num_classes=output_shape)

            train_split = SequenceEagerZippedDataset([BatchedRandomisedDataset(training_set_device_X, batch_size, seed=42), BatchedRandomisedDataset(training_set_device_y, batch_size, seed=42)], stack_batches=False)
            val_split = SequenceEagerZippedDataset([BatchedRandomisedDataset(val_set_x, batch_size, seed=42), BatchedRandomisedDataset(val_set_y, batch_size, seed=42)], stack_batches=False)
            ...
            full_eval_best_model_file_name = f"{working_directory}/model_best.hdf5"
...
if __name__ == '__main__':
    ...
    if args.ft is True:
        trained_model_save_path = os.path.join("./runs", args.pt_store, "Shoaib", "model_best.hdf5")
    else:
