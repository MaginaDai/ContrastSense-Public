...
def train_auxi(args, train_loader, tune_loader, DEVICE, model, optimizer, e, false_optimizer):
    model.train()
    train_loss = 0
    epoch_class_y_loss = 0
    total = 0

    dataset = zip(tune_loader, train_loader)
    for (sensor, target), (sensor_domain, target_domain) in dataset:
        x = torch.concat([sensor, sensor_domain], dim=0).to(DEVICE)
        y = target[:, 0].to(DEVICE)
        d = target_domain[:, 0].to(DEVICE)
...
def train_GILE(model, DEVICE, optimizer, train_loader, tune_loader, val_loader, test_loader, args):
    ...
    logging.info(f"Training with gpu: {not args.disable_cuda}.")
    ...
    for e in range(args.n_epoch):
        ...
        # training 
        avg_epoch_loss, avg_epoch_class_y_loss = train_auxi(args, train_loader, tune_loader, DEVICE, model, optimizer, e, false_optimizer)
        ...
        writer.add_scalar('train_f1_y', train_f1_y, global_step=e)
        writer.add_scalar('val_f1_y', val_f1_y, global_step=e)
