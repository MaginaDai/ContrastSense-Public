...
class qzd(nn.Module):
    ...
    def forward(self, x):
        ...
        zd_loc = self.fc11(out) # extract mean
        zd_scale = self.fc12(out) + 1e-7  # extract delta

        return zd_loc, zd_scale, [idx1, idx2, idx3, idx4], [size1, size2, size3, size4]


class qzx(nn.Module):
    # Encoder for domain-Specific features 
    def __init__(self, d_dim, x_dim, y_dim, zd_dim, zx_dim, zy_dim, args):
        ...
        self.fc11 = nn.Sequential(nn.Linear(512, zx_dim))
        self.fc12 = nn.Sequential(nn.Linear(512, zx_dim), nn.Softplus())
...
class qzy(nn.Module):
    # Encoder for domain-agnostic features 
    def __init__(self, d_dim, x_dim, y_dim, zd_dim, zx_dim, zy_dim, args):
...
# Auxiliary tasks
class qd(nn.Module): # classify domains 
...
class qy(nn.Module): # classify motions 
    def __init__(self, d_dim, x_dim, y_dim, zd_dim, zx_dim, zy_dim):
...
class GILE(nn.Module):
    ...
    def forward(self, d, x, y):
        ...
        zd_q = qzd.rsample()  # get the feature for domain
        if self.zx_dim != 0:
        ...
        zy_q = qzy.rsample()  # get the feature for motion
        ...
        # Auxiliary losses
        d_hat = self.qd(zd_q) # use feature to predict domain
        y_hat = self.qy(zy_q) # use feature to predict motion
