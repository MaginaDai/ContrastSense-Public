...
class ClassifierGRU(nn.Module):
    def __init__(self, cfg, input=None, output=None, feats=False, dataset_name='HHAR'):
        super().__init__()
        for i in range(cfg.num_rnn):
            if input is not None and i == 0:
        ...
        for i in range(cfg.num_linear):
            if output is not None and i == cfg.num_linear - 1:
                ...
                if dataset_name == 'Shoaib':
                    self.__setattr__('lin' + str(i), nn.Linear(cfg.linear_io[i][0], 7))
                elif dataset_name == 'ICHAR':
                    self.__setattr__('lin' + str(i), nn.Linear(cfg.linear_io[i][0], 9))
                else:
                    self.__setattr__('lin' + str(i), nn.Linear(cfg.linear_io[i][0], cfg.linear_io[i][1]))
        self.activ = cfg.activ
        self.dropout = cfg.dropout
...
def fetch_classifier(method, model_cfg, input=None, output=None, feats=False, dataset_name='HHAR'):
    if 'lstm' in method:
        model = ClassifierLSTM(model_cfg, input=input, output=output)
    elif 'gru' in method:
        model = ClassifierGRU(model_cfg, input=input, output=output, dataset_name=dataset_name)
    elif 'dcnn' in method:
