...
from baseline.COCOA.COCOA import COCOA, COCOA_model, Encoder
from baseline.COCOA.dataload import COCOA_Dataset
...
parser.add_argument('-weight', default=1, type=float, help='softmax temperature (default: 0.1)')
...
def main():
    ...
    dataset = COCOA_Dataset(transfer=False, version=args.version, datasets_name=args.name)
    ...
    with torch.cuda.device(args.gpu_index):
        cocoa = COCOA(model=model, optimizer=optimizer, args=args)
        cocoa.train(train_loader)
