...
from utils import accuracy, evaluate, save_checkpoint, save_config_file
...
class SimCLR(object):
    ...
    def sim(self, p1, p2, t):
        ...
        p1_clu, self.cluster_centers, num_record, center_shift = kmeans(
    ...
    def cluster_loss_optimized(self, features):
        ...
        p_clu, self.cluster_centers, num_record, center_shift = kmeans(
    ...
    def transfer_train(self, tune_loader, val_loader):
        ...
        for epoch_counter in tqdm(range(self.args.epochs)):
            ...
            self.scheduler.step()
            logging.debug(f"Epoch: {epoch_counter} \t Loss: {loss} \t train acc: {acc_batch.avg} \t val acc: {val_acc}")
