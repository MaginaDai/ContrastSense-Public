...
from ContrastSense import ContrastSense_model, ContrastSense_v1, ContrastSense_encoder, ContrastSense
...
from utils import ContrastSense_evaluate
from torchvision.transforms import transforms
from data_loader import imu_transforms, emg_transforms, eeg_transforms
...
parser.add_argument('--mol', default='ContrastSense', type=str, help='which model to use', choices=['SimCLR', 'LIMU', 'CPC', 'ContrastSense', 'DeepSense'])
...
parser.add_argument('-penalty', default=False, type=bool, help='Use penalty or not')
...
parser.add_argument('-ContrastSense_m', default=0.999, type=float, help='keys size')
...
def main(args, fisher=None):
    ...
    model = ContrastSense_model(transfer=True, classes=num_of_class, dims=args.d, 
    ...
    with torch.cuda.device(args.gpu_index):
        ...
        if args.evaluate:
            ...
            return
        if args.penalty:
            ContrastSense.transfer_train_penalty(tune_loader, val_loader, fisher)
        else:
            ContrastSense.transfer_train(tune_loader, val_loader)
        best_model_dir = os.path.join(ContrastSense.writer.log_dir, 'model_best.pth.tar')
        ContrastSense.test_performance(best_model_dir=best_model_dir, test_loader=test_loader)
