...
def main():
    ...
    parser.add_argument("--data_dir", type=str, default="physionet.org/files/sleep-edfx/1.0.0/sleep-cassette",
    ...
    domain_list=[]
    sleepEDF_domain=np.arange(20)
    for i in range(len(psg_fnames)):
        if int(psg_fnames[i][-11]) == 2:  ## only sample the first night
            continue 
        domain = int(psg_fnames[i][-13:-11])
        if domain not in sleepEDF_domain:
            continue
        ...
        for a in ann[0]:
            ...
            if label != UNKNOWN:
                ...
                label_epoch = np.ones(duration_epoch) * label
                labels.append(label_epoch)
                idx = int(onset_sec * sampling_rate) + np.arange(int(duration_sec * sampling_rate))
        ...
        for j in range(x.shape[0]):
            ...
            np.savez(loc, eeg=x[j], add_infor=np.array([y[j], domain, time_label]))
            num += 1
            time_label += 1
